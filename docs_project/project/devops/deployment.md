...
5. The script synchronizes production dependencies from the committed
   `uv.lock`, compiles `src/nome`, installs the managed systemd unit, and
   restarts `nome.service`.
